...
int render(framebuffer_t *frame, zbuffer_t *z)
{
	...
	for (;;)
	{
		...
		if (pad_btn_held(&pad, PAD_CROSS))
		{
			struct analog_output analog = read_analog_stick(&pad, ANALOG_STICK_LEFT);

			// Spin the cube a bit.
			object_rotation[0] += 0.012f * analog.vertical;	  // while (object_rotation[0] > 3.14f) { object_rotation[0] -= 6.28f; }
			object_rotation[1] += 0.012f * analog.horizontal; // while (object_rotation[1] > 3.14f) { object_rotation[1] -= 6.28f; }
		}
